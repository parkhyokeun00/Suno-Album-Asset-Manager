...
const FS = {
            ...
            if (entry.kind === 'file' && isAudioFile(entry.name)) {
                ...
                if (forceUpdate || !songData) {
                    try {
                        ...
                        // Enforce Album -> Folder mapping if available
                        // Logic: If ID3 Album exists, that IS the folder.
                        if (id3 && id3.album) {
                            const albumFolder = id3.album.trim();
                            // Prevent creating folders that are just the filename (common bad ID3 tagging)
                            const simpleName = entry.name.replace(/\.[^/.]+$/, "");

                            if (albumFolder && albumFolder !== simpleName) {
                                if (!State.folders.includes(albumFolder)) State.folders.push(albumFolder);
                                songData.folder = albumFolder;
                            }
                        } else if (songData.folder === 'Unsorted') {
                            // If no Album and currently Unsorted, remains Unsorted
                        }

                        // Restore Local Cover (Moved outside to run for existing songs too)
                ...
                // ALWAYS Attempt to restore local cover if needed (and not already set by ID3 above)
                if (songData.coverPath && songData.coverPath.startsWith('covers/') && !songData.coverUrl) {
                    try {
                        // Optimization: Cache directory handle?
                        // For now just get it.
                        const coversDir = await State.dirHandle.getDirectoryHandle('covers');
                        const fh = await coversDir.getFileHandle(songData.coverPath.split('/')[1]);
                        const file = await fh.getFile();
                        songData.coverUrl = URL.createObjectURL(file);
                    } catch (e) {
                        // console.warn("Failed to load local cover", songData.coverPath);
                    }
                }

                // Fallback: If NO coverUrl (because excluded in JSON) and NO local coverPath, 
                // it means it should use the original ID3 cover. 
                // We must read it now (Soft Scan didn't read it yet).
                if (!songData.coverUrl && !songData.coverPath) {
                    try {
                        const file = await entry.getFile();
                        const id3 = await readID3(file);
                        if (id3 && id3.coverUrl) {
                            songData.coverUrl = id3.coverUrl;
                        }
                    } catch (e) {
                        // Ignore read errors
                    }
    ...
    async saveVault() {
        ...
        const dataToSave = {
            ...
            songs: State.songs.map(s => {
                const { handle, audioUrl, coverUrl, status, ...rest } = s; // Exclude coverUrl (dead blob)
                return rest;
            })
        };
    ...
    async importFile() {
        ...
        try {
            ...
            // Read ID3 to determine Folder
            const id3 = await readID3(file);
            let folderName = 'Unsorted';
            if (id3 && id3.album) {
                folderName = id3.album.trim() || 'Unsorted';
            }
            ...
            // Copy File to Vault Root (or maybe subfolders later? For now Flat Vault structure as per existing app)
            // Existing app seems to read flat directory. 
            // So we just copy file to root.
            const newHandle = await State.dirHandle.getFileHandle(file.name, { create: true });
            const writable = await newHandle.createWritable();
            await writable.write(file);
            await writable.close();

            UI.showToast(`Imported to '${folderName}'`);

            // Trigger Scan/Update logic for this single file or full scan?
            // Full scan is safest to reuse logic.
            await this.scanVault(false);
...
const AudioEngine = {
    ...
    init() {
        ...
        this.el.addEventListener('timeupdate', () => {
            ...
            const fBar = $('#footer-progress-bar');
        ...
        // Loop Listener (Optional, user didn't ask but good for single track repeat if needed)
        // this.el.loop = false; 
        ...
        setTimeout(() => {
            ...
            // Footer Seek
            const fSeek = $('#footer-progress-area');
            if (fSeek) {
                fSeek.onclick = (e) => {
                    const rect = fSeek.getBoundingClientRect();
                    const x = e.clientX - rect.left;
                    const pct = x / rect.width;
                    if (this.el.duration) this.el.currentTime = this.el.duration * pct;
                };
            }

            // Footer Volume
            const fVol = $('#footer-volume');
            if (fVol) {
                fVol.oninput = (e) => this.setVolume(e.target.value);
                // Initialize volume
                fVol.value = this.el.volume;
            }

            // Close Button
            const closeBtn = $('#btn-close-footer');
            if (closeBtn) {
                closeBtn.onclick = () => {
                    this.el.pause();
                    $('#footer-player').classList.add('translate-y-full');
                };
            }
        }, 100);
...
const UI = {
    ...
    async deleteSong(id) {
        const songIdx = State.songs.findIndex(s => s.id === id);
        if (songIdx === -1) return;

        const song = State.songs[songIdx];

        if (confirm(`Warning: This will delete the file "${song.filename}" from your local disk. This cannot be undone.\n\nAre you sure?`)) {
            try {
                // Delete from Disk
                await State.dirHandle.removeEntry(song.filename);

                // Remove from State
                State.songs.splice(songIdx, 1);
                ...
                // Render
                this.render();
                this.showToast("File deleted from disk.");

            } catch (e) {
                console.error("Failed to delete file", e);
                this.showToast("Error deleting file: " + e.message);
            }
        }
    },
    ...
    showSongContextMenu(e, song) {
        ...
        // Handlers
        const deleteBtn = $('#song-ctx-delete');
        if (deleteBtn) {
            deleteBtn.onclick = () => {
                menu.classList.add('hidden');
    ...
    renderGrid() {
        ...
        filtered.forEach(s => {
            ...
            el.oncontextmenu = (e) => {
                e.preventDefault();
                e.stopPropagation(); // Prevent folder context menu if bubbling
                this.showSongContextMenu(e, s);
            };
...
$('#btn-refresh').onclick = () => FS.scanVault(false);
$('#btn-import-file').onclick = () => FS.importFile();
